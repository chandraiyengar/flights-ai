...
const SearchFlightsParams = z.object({
    ...
    trip: z
        .enum(['one-way', 'round'])
        .default('one-way')
        .describe('Trip type: one-way or round'),
    seat: z
        .enum(['economy', 'business', 'first'])
        .default('economy')
        .describe('Seat class preference'),
    ...
    infantsInSeat: z
        ...
        .default(0)
        .optional()
        .describe('Number of infants requiring their own seat'),
    infantsOnLap: z
        .number()
        .min(0)
        .default(0)
        .optional()
        .describe('Number of lap infants'),
    maxStops: z
        .number()
        .min(0)
        .default(0)
        .optional()
        .describe('Maximum number of stops allowed'),
});
...
export async function POST(req: Request) {
    ...
    const result = streamText({
        ...
        system:
            'You are a helpful assistant that can search for flights and provide information about them.' +
        ...
        tools: {
            ...
            getAirports: tool({
                ...
                parameters: z.object({
                    location: z
                        .string()
                        .describe('The location to get the airports for'),
                }),
                execute: async ({ location }) => {
                    const response = await fetch(
                        `http://localhost:8000/airports?location=${location}`,
                    );
            ...
            findPlaceCode: tool({
                description:
                    'Find the place code for a location which has more than one airport.',
                parameters: z.object({
                    location: z
                        .string()
                        .describe('The location to find the place code for'),
                }),
                execute: async ({ location }) => {
